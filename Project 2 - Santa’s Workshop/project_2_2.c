...
int main(int argc,char **argv){
    ...
    */
    ...
    while(1){
        ...
        if((time(NULL) - simulation_started != 0) && ((time(NULL) - simulation_started) % emergencyFrequency == 0)){
            emergency_flag = 1;
        }
        ...
        }else if(random_probability > 0.5 && random_probability <= 0.7){
        ...
        }else if(random_probability > 0.7 && random_probability <= 0.9){
        ...
        }else if (random_probability > 0.9 && random_probability <= 0.95){
...
void* Santa(void *arg){
    while(1){
        ...
        //Quality assurence - b option in Part II is prior.
        // 3 or more GameStations are waiting to go through QA.
        pthread_mutex_lock(&q_a_mutex);
        if(quality_assurance_queue->size >= 3){
            ret = Dequeue(quality_assurance_queue);
            qa_flag = 1;
        }
        pthread_mutex_unlock(&q_a_mutex);

        //if quality ass is not proceeded, check delivery.
        if(qa_flag == 0){
            //Delivery
            pthread_mutex_lock(&delivery_mutex);
        ...
        //Quality assurence --else statement of Delivery(line 367)
